  ...
  const loadTrends = useCallback(async (forceDiscover = false) => {
    ...
    try {
      if (forceDiscover && trendFilters.industry) {
        // Use the new discover endpoint to find trends for the selected industry
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Choose the industry category to discover trending topics from
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            {form.watch("generationType") === "trending_select" && (
              <div className="w-2/3 space-y-4 overflow-y-auto pl-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-semibold">Select Trending Topics</h3>
                </div>
                <TrendSelector
                  trends={trends}
